...
Bu proje, yaz stajı kapsamında geliştirilmekte olan, FPS (Birinci Şahıs) bakış açısına sahip bir 3D hayatta kalma (survival) oyunu prototipidir. Oyuncu, sınırları belirlenmiş küçük bir alanda kaynaklarını doğru yöneterek temel istatistiklerini (Can, Açlık, Susuzluk) dengede tutmaya çalışır.
...
* Fare (Mouse): Etrafa Bakma / Kamera Kontrolü
...
* Karakter Kontrolcüsü: Fizik kurallarına ve yerçekimine uygun çalışan, kamera açısı (Pitch) sınırlandırılmış FPS karakter donanımı.
